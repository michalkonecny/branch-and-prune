...
module BranchAndPrune.Logging (BPLogConfig (..), defaultBPLogConfig) where
...
instance (MonadIO m, MonadLogger m, A.ToJSON problem, A.ToJSON paving) => HasLoggingFunctions (Step problem paving) m where
  type LoggingConfig (Step problem paving) = BPLogConfig
  type LoggingResources (Step problem paving) = BPLogResources
